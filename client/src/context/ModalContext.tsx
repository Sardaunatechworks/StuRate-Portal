    ...
    <ModalContext.Provider value={{ showFeedback, showConfirm, hideModal }}>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
          <div className="bg-white dark:bg-[#142620] rounded-3xl p-6 w-full max-w-md border border-zinc-200/90 dark:border-academic-800 shadow-2xl space-y-5">
            ...
            <div className="flex items-start justify-between">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm shrink-0 font-bold">
                ...
                {modalData.type === 'info' && (
                  <div className="w-12 h-12 rounded-2xl bg-academic-100 dark:bg-academic-800 text-academic-800 dark:text-academic-200 flex items-center justify-center">
                    <Info size={24} />
                  </div>
                )}
              </div>
              ...
              <button
                ...
                className="p-1.5 rounded-xl text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-academic-800 transition-colors"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-1.5">
              <h3 className="text-lg font-extrabold text-zinc-900 dark:text-white leading-tight">{modalData.title}</h3>
              <p className="text-xs text-zinc-600 dark:text-zinc-300 leading-relaxed font-normal">{modalData.message}</p>
            </div>

            {/* Modal Action Buttons */}
            <div className="pt-2 flex items-center justify-end gap-3 border-t border-zinc-100 dark:border-academic-800">
              {modalData.type === 'confirm' ? (
                <>
                  <button
                    type="button"
                    disabled={isSubmitting}
                    onClick={hideModal}
                    className="px-4 py-2.5 text-xs font-semibold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-academic-800 rounded-xl transition-all"
                  >
                  ...
                  <button
                    ...
                    className="bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl px-5 py-2.5 text-xs transition-all shadow-md"
                  >
                ...
                <button
                  type="button"
                  onClick={handleConfirmAction}
                  className="w-full template-btn-primary py-2.5 text-xs font-bold shadow-md"
                >
