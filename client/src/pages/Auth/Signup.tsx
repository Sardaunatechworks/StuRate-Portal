...
export const Signup: React.FC = () => {
  ...
  const fetchDepartments = async () => {
    ...
    try {
      const res = await API.get('/departments').catch(() => API.get('/auth/departments'));
      ...
      if (rawList.length > 0) {
        ...
        const validFromApi = (rawList as DepartmentOption[])
        ...
        // Merge with FALLBACK_DEPARTMENTS if needed so all 4 departments are always present
